...
class Node:
    ...
    def handle_block(self, block):
        ...
        # valida la transaccion coinbase, es la numero 0 de cada block
...
# prepara el bloque recompensa para los mineros?
def prepare_coinbase(public_key, tx_id=None):
    ...
    return Tx(
        ...
        tx_outs=[
            TxOut(tx_id=tx_id, index=0, amount=BLOCK_SUBSIDY, public_key=public_key),
        ],
    )
...
def mine_forever(public_key):
    ...
    while True:
        coinbase = prepare_coinbase(public_key=public_key)
        unmined_block = Block(
...
def mine_genesis_block(public_key):
    global node
    coinbase = prepare_coinbase(public_key=public_key, tx_id="abc123")
...
def read_message(s):
    message = b''
    # our protocol is: first 4 bytes signify messages length
...
class TCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        if command == "sync":
            peer_blocks_ids = data
            for block in node.blocks[::-1]:
                if block.id not in peer_blocks_ids and block.prev_id in peer_blocks_ids:
...
def lookup_private_key(keyword):
    ...
    # Otherwise, look up in this little registry of "characters"
        ...
        exponent = {"alice": 1, "bob": 2, "joan": 3, "tomas": 4}[keyword]
...
def main(args):
    if args["serve"]:
        ...
        duration = 10 * ["node0", "node1", "node2"].index(hostname)
        time.sleep(duration)
        ...
        # Start miner thread
        miner_public_key = lookup_public_key(username)
