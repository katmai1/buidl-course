...
class Node:
    ...
    def handle_block(self, block):
        # ignorar si ya se ha comprobado este bloque
        ...
        if found_in_chain or found_in_branch:
            raise Exception("Receive duplicate block")

        # buscamos el bloque previo
        ...
        # valida proof of work sempre, las transacciones solo si se extiende la mainchain
        self.validate_block(block, validate_txns=extends_chain)
        
        # handle per cada condicion por separado
        if extends_chain:
            ...
            # reorg si la rama es mejor que la mainchain
    ...
    def disconnect_tx(self, tx):
        ...
        if not tx.is_coinbase:
            for tx_in in tx.tx_ins:
                ...
                self.utxo_set[tx_out.outpoint] = tx_out
        # remove UTXOs created by this transactions
        for tx_out in tx.tx_outs:
